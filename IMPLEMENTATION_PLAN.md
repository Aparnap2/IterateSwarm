...
> **V6 Frozen 9-Runtime Model (see `docs/v6-runtime-architecture.md` v3.0):** The target architecture is the **Enterprise Decision Runtime** — 5 layers (Observe → Understand → Decide → Act → Learn), **9 Runtimes** (Evidence, Knowledge, Decision Context, Policy, Workspace, Action, Simulation, Learning, reserved), and agents are an implementation detail inside runtimes. This supersedes the "Shared Platform Agent + 5 Skills" framing in `PLAN_V6.md` ADR-004 and the V5.2 agent roster. The execution workstreams below remain valid; they now build toward the 9-runtime model. Key additions: the **Decision Context Runtime** (minimal high-signal context per decision, `docs/decision-context-runtime.md`), the **Policy Runtime** (deterministic gate: Validation → Policy → Governance → Execution), and the **Simulation Runtime** (placeholder — executive what-if). Governance is a deterministic policy gate, not an agent; the Execution agent never reasons.
>
> **V6 Frozen Platform Stack (see `docs/ADR-009-platform-runtime-freeze.md`):** Production = Google Cloud (Cloud Run, FastAPI, Temporal, Redpanda, Cloudflare R2, Neon PostgreSQL, BigQuery Sandbox analytics-only, Neo4j Aura, **pgvector primary**, Enterprise Retrieval = BM25 + pgvector + Neo4j traversal + metadata filters + reranking, Gemini/Groq/OpenRouter, Sarvam OCR/Docling, OpenTelemetry → Langfuse → Sentry, Flagsmith, Better Auth, Resend). Local dev = **MiniSky** (local GCP control plane, API gateway `http://localhost:8080`, console `http://localhost:8081` — emulates Cloud Storage, Pub/Sub, BigQuery, IAM, LRO, Cloud Run API) + Docker Compose (Neo4j, Postgres, Temporal, Redpanda, Redis, Langfuse) + Mockoon cli (ports 3001–3004). **pgvector-primary** — **Qdrant removed** for MVP (optional later behind the same VectorStore interface); **MinIO removed** (MiniSky covers local GCS). Also removed: Spark, Cloud SQL, Cloud Workflows. Decision Store / Feature Store / Knowledge Cache are deferred to V6.5.
